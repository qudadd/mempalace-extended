...
from .asset_store import is_image_file, store_image_asset
from .document_loader import SUPPORTED_EXTENSIONS, read_source_text
...
def detect_room(filepath: Path, content: str, rooms: list, project_path: Path) -> str:
    ...
    # Priority 2: filename matches room name or room keywords
    for room in rooms:
        candidates = [room["name"].lower()] + [k.lower() for k in room.get("keywords", [])]
...
def delete_drawers_for_source_file(collection, source_file: str) -> int:
    """Delete all drawers previously filed from one source file."""
    try:
...
def prune_missing_drawers(
    ...
    """Delete drawers whose source files no longer belong to the current scan."""
...
def file_already_mined(collection, source_file: str) -> bool:
    ...
    try:
        ...
        if is_image_file(source_file) and not stored_meta.get("asset_path"):
            return False
...
def process_file(
    ...
    if is_image_file(filepath):
        extra_metadata = store_image_asset(filepath, palace_path)
    ...
    for root, dirs, filenames in os.walk(project_path):
        ...
        for filename in filenames:
            ...
            if not force_include and filename in SKIP_FILENAMES:
                continue
            if filepath.suffix.lower() not in SUPPORTED_EXTENSIONS and not exact_force_include:
                continue
