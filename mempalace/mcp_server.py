...
def tool_search(query: str, limit: int = 5, wing: str = None, room: str = None):
    ...
    images = _find_related_images(query, results, wing=wing, limit=3)
    return _multimodal_search_response(results, images)


def tool_get_asset(name: str, wing: str = None, limit: int = 3):
    """Fetch stored image assets by filename or figure reference."""
    col = _get_collection()
    if not col:
        return _no_palace()
    ...
    try:
        ...
        if wing:
            kwargs["where"] = {"wing": wing}
        metas = col.get(**kwargs).get("metadatas", [])
    except Exception as e:
        return {"error": str(e)}

    images = _match_images(name, metas, limit=limit)
    summary = {
        "query": name,
        "wing": wing,
        "images": [_asset_summary(meta) for meta in images],
    }
    content = [{"type": "text", "text": _render_asset_summary(summary)}]
    content.extend(_build_image_blocks(images))
    return {"_mcp_content": content, "_structured": summary}
...
def _match_images(query: str, metas: list, limit: int = 3) -> list:
    query_lower = query.lower().strip()
    refs = _extract_figure_refs(query)
    scored = []

    for meta in metas:
        if not _is_image_meta(meta):
            continue
        filename = Path(meta.get("source_file", meta.get("asset_path", ""))).name.lower()
        score = 0

        if query_lower and query_lower in filename:
            score += 10
        for ref in refs:
            if _filename_matches_figure(filename, ref):
                score += 8
        if ("figure" in query_lower or "fig" in query_lower or "图" in query_lower) and score == 0:
            if meta.get("room") == "figures":
                score += 2

        if score > 0:
            scored.append((score, filename, meta))

    if any(score >= 8 for score, _, _ in scored):
...
def _find_related_images(query: str, search_results: dict, wing: str = None, limit: int = 3) -> list:
    ...
    matched = _match_images(query, metas, limit=limit)
    if matched:
        return matched

    text = " ".join(hit.get("text", "")[:500] for hit in search_results.get("results", []))
    refs = _extract_figure_refs(query + "\n" + text)
    if not refs:
        return []

    image_metas = [meta for meta in metas if _is_image_meta(meta)]
    scored = []
...
def _filename_matches_figure(filename: str, figure_ref: str) -> bool:
    patterns = [
        rf"figure[\s_-]*{re.escape(figure_ref)}",
        rf"fig[\s._-]*{re.escape(figure_ref)}",
        rf"图[\s_-]*{re.escape(figure_ref)}",
    ]
...
def _extract_figure_refs(text: str) -> set[str]:
    ...
    for pattern in patterns:
        refs.update(re.findall(pattern, text, flags=re.IGNORECASE))
    return refs
...
def _resolve_asset_path(meta: dict) -> str | None:
    asset_path = meta.get("asset_path")
    if asset_path and Path(asset_path).exists():
        return asset_path
    source_file = meta.get("source_file")
    if source_file and Path(source_file).exists() and is_image_file(source_file):
        return source_file
    return None


def _asset_summary(meta: dict) -> dict:
    asset_path = _resolve_asset_path(meta)
    return {
        "wing": meta.get("wing"),
        "room": meta.get("room"),
        "source_file": meta.get("source_file"),
...
def _build_image_blocks(images: list) -> list:
    blocks = []
    for meta in images:
        asset_path = _resolve_asset_path(meta)
        if not asset_path:
        ...
        blocks.append(
            {
                "type": "image",
                "data": encode_image_asset(asset_path),
                "mimeType": mime_type,
            }
        )
    return blocks
...
def _render_asset_summary(summary: dict) -> str:
    ...
    for index, image in enumerate(images, 1):
        lines.append(f"[{index}] {Path(image['source_file']).name}")
        lines.append(f"    room: {image.get('room')}")
        if image.get("asset_path"):
            lines.append(f"    asset: {image['asset_path']}")
    return "\n".join(lines)


def _multimodal_search_response(results: dict, images: list) -> dict:
    content = [{"type": "text", "text": _render_search_summary(results, images)}]
    content.extend(_build_image_blocks(images))
    structured = dict(results)
    structured["images"] = [_asset_summary(meta) for meta in images]
    response = dict(results)
    response["images"] = structured["images"]
    response["_mcp_content"] = content
    response["_structured"] = structured
    return response


def _render_search_summary(results: dict, images: list) -> str:
    ...
    for index, hit in enumerate(hits, 1):
        lines.append(f"[{index}] {hit.get('wing')}/{hit.get('room')} :: {hit.get('source_file')}")
        lines.append(f"    similarity: {hit.get('similarity')}")
    ...
    if images:
        lines.append(f"Related images: {len(images)}")
        for index, meta in enumerate(images, 1):
            source_name = Path(meta.get("source_file", meta.get("asset_path", ""))).name
            lines.append(f"[img {index}] {source_name} ({meta.get('room')})")
    return "\n".join(lines)
...
TOOLS = {
    ...
    "mempalace_search": {
        "description": "Semantic search. Returns verbatim drawer content with similarity scores, and when relevant also returns related stored image assets alongside the text results.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mempalace_get_asset": {
        "description": "Fetch stored image assets by filename, figure number, or image query. Returns text metadata together with the actual image content when available.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    if method == "initialize":
        ...
        try:
            ...
            if isinstance(result, dict) and "_mcp_content" in result:
                response_result = {"content": result["_mcp_content"]}
                structured = result.get("_structured")
                if structured is not None:
                    response_result["structuredContent"] = structured
                return {
                    "jsonrpc": "2.0",
                    "id": req_id,
