...
def test_mine_refreshes_changed_files_and_prunes_missing():
    tmpdir = tempfile.mkdtemp()
    try:
        project_root = Path(tmpdir).resolve()

        write_file(project_root / "backend" / "app.py", "print('alpha')\n" * 40)
        write_file(project_root / "docs.md", "beta\n" * 40)
        with open(project_root / "mempalace.yaml", "w") as f:
            yaml.dump(
                {
                    "wing": "test_project",
        ...
        time.sleep(1.1)
        write_file(project_root / "backend" / "app.py", "print('gamma')\n" * 40)
        os.remove(project_root / "docs.md")

        mine(str(project_root), str(palace_path), prune_missing=True)
        ...
        source_names = {Path(meta["source_file"]).name for meta in metas}

        assert "docs.md" not in source_names
        assert any("gamma" in doc for doc in docs)
        assert not any("alpha" in doc for doc in docs)
        del col
        del client
        gc.collect()
    finally:
